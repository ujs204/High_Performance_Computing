...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = 2 * n * k * m * NREPEATS / time / 1e9; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = 4 * (n * k * m / BLOCK_SIZE) * NREPEATS * sizeof(double)/time/1e9; // TODO: calculate from m, n, k, NREPEATS, time
